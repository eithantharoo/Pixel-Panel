...
function MetaList({ metadata, rating }) {
  const rows = metadata ?? DEFAULT_METADATA;
    ...
    <dl className="mt-4 space-y-1 text-base leading-[1.7] sm:text-lg">
      ...
      <div className="grid grid-cols-[7.5rem_minmax(0,1fr)] items-center gap-x-2 sm:grid-cols-[8.5rem_minmax(0,1fr)]">
        ...
        <dd className="flex items-center gap-1.5 font-normal text-[var(--home-accent)]">
          ...
          {typeof displayRating === 'number' ? displayRating.toFixed(1) : displayRating}/10
        </dd>
      </div>
    </dl>
  );
  ...
  if (book) {
    return (
      ...
      >
        ...
        {book.rank != null && (
          <span className="absolute left-3 top-3 flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--home-accent)] text-sm font-extrabold text-[var(--home-ink)] shadow">
            {book.rank}
          </span>
        )}
      </div>
...
export default function HeroCard({ isReading, onReadNow, onClose, book, isFavorite = false, onToggleFavorite }) {
  ...
  return (
    <div key={title} className="relative flex h-full min-h-0 flex-col" style={{ animation: 'heroFadeIn 0.35s ease' }}>
      ...
      <div className="grid items-start gap-x-6 gap-y-0 pr-11 sm:grid-cols-[minmax(0,220px)_minmax(0,1fr)] lg:grid-cols-[minmax(0,240px)_minmax(0,1fr)]">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            >
              <Heart size={14} strokeWidth={2.3} fill={isFavorite ? 'currentColor' : 'none'} />
      ...
      <div className="mt-7">
        ...
        <GenreList genres={genres} light />
      </div>

      <div className="mt-7 mb-0">
        <h3 className="mb-4 text-xl font-bold text-[var(--home-accent)] sm:text-2xl">Review</h3>
        <p className="text-base leading-[1.75] text-[var(--home-text)] sm:text-lg">{review}</p>
      </div>
    </div>
  );
}
