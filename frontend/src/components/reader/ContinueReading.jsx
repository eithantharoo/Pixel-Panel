        ...
        {books.map((book, index) => (
          <React.Fragment key={book.title}>
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <img src={book.img} alt={book.title} className="w-11 h-11 rounded object-cover shrink-0 border-2 border-[var(--purple-normal)]" />
              <div className="flex-1 min-w-0">
                <h4 className="text-[13px] font-bold text-white leading-tight truncate">{book.title}</h4>
                <p className="text-[11px] text-[var(--text-muted)] mb-1.5 font-medium">chapter {book.chapter}</p>
