        ...
        {otherTrends.map((item) => (
          <div key={item.id} className="flex items-center gap-3 relative">
            ...
            <img
              src={item.img}
              alt={item.title}
              className="w-[85px] h-14 object-cover rounded ml-3 border border-white/10 shadow-sm"
            />
            <div>
              <h4 className="text-[13px] font-bold text-[var(--text-yellow)] leading-tight">
                {item.title}
              </h4>
              <p className="text-[11px] text-white">
                <span className="text-[var(--text-yellow)]">★</span> {item.rating}
              </p>
            </div>
          </div>
        ))}
